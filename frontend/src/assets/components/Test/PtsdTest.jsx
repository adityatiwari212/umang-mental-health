...
import { ptsdQuestions } from '../../../constants';
...
function PtsdTest() {
    ...
    const handleNext = () => {
        if (responses[currentQuestionIndex] === undefined) {
            setWarning(true);
        } else if (currentQuestionIndex < Object.keys(ptsdQuestions).length - 1) {
    ...
    const sendResponsesToBackend = async (ansArr) => {
        try {
            const response = await axios.post('/api/ptsd-test', { responses: ansArr }); // Update with your backend endpoint
    ...
    const currentQuestion = Object.entries(ptsdQuestions)[currentQuestionIndex];
    ...
    return (
        <div className='AnxietyTest-container'>
            ...
            <div className='intro'>
                <p>This test will help you assess your ptsd levels by answering a series of questions. Please select the option that best describes how you feel.</p>
            </div>
            {!showResult ? (
                <>
                    ...
                    <div className="button-container-right-end">
                        <button className='button-50' onClick={handleNext}>
                            {currentQuestionIndex < Object.keys(ptsdQuestions).length - 1 ? "Next→" : "Submit"}
                        </button>
                    </div>
                </>
            ) : (
