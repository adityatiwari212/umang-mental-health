...
import { anxietyQuestions } from '../../../constants';
...
function AnxietyTest() {
    ...
    const handleNext = () => {
        if (responses[currentQuestionIndex] === undefined) {
            setWarning(true);
        } else if (currentQuestionIndex < Object.keys(anxietyQuestions).length - 1) {
    ...
    const sendResponsesToBackend = async (ansArr) => {
        try {
            const response = await axios.post('/api/anxiety-test', { responses: ansArr }); // Update with your backend endpoint
    ...
    const currentQuestion = Object.entries(anxietyQuestions)[currentQuestionIndex];
    ...
    return (
        <div className='AnxietyTest-container'>
            ...
            <div className='intro'>
                <p>This test will help you assess your anxiety levels by answering a series of questions. Please select the option that best describes how you feel.</p>
            </div>
            {!showResult ? (
                <>
                    ...
                    <div className="button-container-right-end">
                        <button className='button-50' onClick={handleNext}>
                            {currentQuestionIndex < Object.keys(anxietyQuestions).length - 1 ? "Next→" : "Submit"}
                        </button>
                    </div>
                </>
            ) : (
