...
function DepressionTest() {
    ...
    const handleNext = () => {
        if (responses[currentQuestionIndex] === undefined) {
            setWarning(true);
        } else if (currentQuestionIndex < Object.keys(DepressionQuestions).length - 1) {
    ...
    const currentQuestion = Object.entries(DepressionQuestions)[currentQuestionIndex];
    ...
    return (
        <div className='AnxietyTest-container'>
            ...
            <div className='intro'>
                <p>This test will help you assess your depression levels by answering a series of questions. Please select the option that best describes how you feel.</p>
            </div>
            {!showResult ? (
                <>
                    ...
                    <div className="button-container-right-end">
                        <button className='button-50' onClick={handleNext}>
                            {currentQuestionIndex < Object.keys(DepressionQuestions).length - 1 ? "Next→" : "Submit"}
                        </button>
                    </div>
                </>
            ) : (
